...
export default function ActivityTile() {
  return (
    <motion.article
      ...
      className="relative rounded-3xl overflow-hidden border border-[#1f1f2e] p-8 group"
    ...
    >
      ...
      <div className="relative z-10 flex flex-col lg:flex-row gap-10">
        ...
        <div className="flex-1 flex flex-col gap-5 min-w-0">
          <div>
            <h3 className="font-display text-base font-semibold" style={{ color: '#f0f0ff' }}>
              Learning Activity
            </h3>
          ...
          <div className="flex items-center gap-2.5">
            <span className="text-[11px]" style={{ color: '#44445a' }}>Less</span>
            {[0,1,2,3,4].map((v) => (
              <div key={v} className="w-3 h-3 rounded-sm" style={{ background: intensityColor(v) }} />
            ))}
            <span className="text-[11px]" style={{ color: '#44445a' }}>More</span>
          </div>
        </div>
          ...
          {stats.map(({ icon: Icon, label, value, color, bg, border }) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium" style={{ color: '#8888aa' }}>{label}</span>
              </div>
